  ...
  return prescriptions.map((p) => {
    ...
    return {
      ...
      items: p.items.map((item) => {
        const alreadyDispensed = dispensedByMedicine.get(item.medicineId) ?? 0;
        return { id: item.id, medicineId: item.medicineId, medicineName: item.medicine.name, strength: item.medicine.strength, prescribed: item.quantity, alreadyDispensed, remaining: Math.max(item.quantity - alreadyDispensed, 0) } satisfies PartialQueueItem;
      }),
    };
  });
}
...
export async function dispensePartialPrescription(context: AuthContext, prescriptionId: string, requested: Record<string, number>) {
  return db.$transaction(async (tx) => {
    ...
    const plan = prescription.items.map((item) => {
      const already = dispensedByMedicine.get(item.medicineId) ?? 0;
      const remaining = item.quantity - already;
      const value = requested[item.id] ?? 0;
      if (!Number.isInteger(value) || value < 0 || value > remaining) throw new Error("INVALID_DISPENSING_QUANTITY");
      return { item, remaining, value };
    }).filter((x) => x.value > 0);
    ...
    const allFulfilled = prescription.items.every((item) => (dispensedByMedicine.get(item.medicineId) ?? 0) + (requested[item.id] ?? 0) >= item.quantity);
