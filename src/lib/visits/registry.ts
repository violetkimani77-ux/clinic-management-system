...
export async function updateVisit(
  ...
  return db.$transaction(async (tx) => {
    ...
    if (input.status && input.status !== current.status && !ALLOWED_TRANSITIONS[current.status].includes(input.status)) {
    ...
    const visit = await tx.visit.update({
      ...
      data: {
        ...
        closedAt: nextStatus === VisitStatus.COMPLETED || nextStatus === VisitStatus.CANCELLED ? new Date() : null,
      },
      select: { id: true, patientId: true, status: true, openedAt: true, closedAt: true, notes: true },
    });
    ...
    await recordAuditEvent(context, {
      ...
      entityType: "Visit",
      entityId: visit.id,
      metadata: { previousStatus: current.status, nextStatus: visit.status },
    }, tx);
    await recordSyncChange(tx, {
      clinicId: context.clinicId,
      entityType: "Visit",
